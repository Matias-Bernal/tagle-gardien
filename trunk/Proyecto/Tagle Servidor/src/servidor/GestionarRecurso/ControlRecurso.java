...
public class ControlRecurso extends UnicastRemoteObject implements IControlRecurso {
	...
	public void modificarRecurso(Long id, RecursoDTO modificado) throws Exception {
		...
		try {
			...
			if(modificado.getPedido_pieza()!=null){
				Pedido_PiezaAssembler pedido_piezaAssemb = new Pedido_PiezaAssembler(accesoBD);
				recurso.setPedido_pieza(pedido_piezaAssemb.getPedido_Pieza(modificado.getPedido_pieza()));
			}
			accesoBD.concretarTransaccion();
		} catch (Exception e) {
			accesoBD.rollbackTransaccion();
	...
	public Vector<RecursoDTO> obtenerRecursos() throws Exception {
		...
		try {
			...
			for (int i = 0; i < recursos.size(); i++) {
				...
				Pedido_PiezaAssembler pedido_piezaAssemb = new Pedido_PiezaAssembler(accesoBD);
				if (recursos.elementAt(i).getPedido_pieza()!=null)
					recursoDTO.setPedido_pieza(pedido_piezaAssemb.getPedido_PiezaDTO(recursos.elementAt(i).getPedido_pieza()));
